...
export const LocationProvider: React.FC<LocationProviderProps> = ({ children }) => {
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      ...
      console.log('📊 LocationContext: Loaded', allLocations.length, 'locations from database');
  ...
  const importLocations = useCallback(async (locationsData: Omit<Location, 'id' | 'created_at' | 'updated_at'>[]) => {
    console.log('🔄 LocationContext: Starting import of', locationsData.length, 'locations');
    setLoading(true);
    setError(null);
    let success = 0;
    let failed = 0;
    ...
    try {
      for (const locationData of locationsData) {
        try {
          ...
          if (newLocation) {
            ...
            success++;
            if (success % 100 === 0) {
              console.log('📊 LocationContext: Imported', success, 'locations so far...');
            }
          } else {
            failed++;
          }
      ...
      console.log('✅ LocationContext: Completed - Success:', success, 'Failed:', failed);
    } catch (err) {
