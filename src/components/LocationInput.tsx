...
interface LocationInputProps {
  onLocationAdd?: (location: { name: string; lat: number; lng: number }) => void;
}
...
const LocationInput: React.FC<LocationInputProps> = ({ onLocationAdd }) => {
  ...
  const handleAddressSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const MAPBOX_TOKEN = 'pk.eyJ1IjoibHVtaWFyaWEiLCJhIjoiY2o4b25kbzYyMDVucTMzcnp2emxhMG1sYiJ9.4V9px9CLMCy6oyNWtKWb6A'; // Replace with your token
      ...
      if (onLocationAdd) {
        onLocationAdd({ name: address, lat: mockCoords.lat, lng: mockCoords.lng });
      }
      setAddress('');
    }
  ...
  const getMockCoordinates = (address: string): { lat: number; lng: number } => {
